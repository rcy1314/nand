...
var feed = function(n) {
  if (n == 0) n = menu.indexOf(menu[
    Math.floor(Math.random() *
      menu.length - 1)])
  else if (n >= menu.length - 13) n =
    1
  for (var i = n + 1; i <= n +
    13; i++) {
    if (menu[i]) var img =
      'images/png/' + menu[i].img +
      '.png'
    $('#main .center .feed').append(
      ...
      "<img src='" + img +
      "' class='id " + menu.indexOf(
        menu[i]) + "' response='" +
      menu[i].id.toLowerCase()
      .replace(/[\/|\.|\s|\-]/g,
        '-') + "' search='" + menu[
        i].cat.toLowerCase() +
      "'> " +
      "<a style='left:0;width:100%' ext='" +
      menu[i].ext +
      "' rel='nofollow'>" + String(
        menu[i].id.match(/[^\/]+$/g)
        ).substring(0, 9) + '...' +
...
var content = function(n, recent,
  oldest, images, posts) {
  var img = 'images/png/' + menu[n]
    .img + '.png'
  $('#main .stats').append(
    "<img src='" + img +
    "' class='id " + menu.indexOf(
    ...
    "<div class='info'>" +
    "<a ext='" + menu[n].ext +
    "' rel='nofollow'>" + menu[n].id
    .match(/[^\/]+$/g) +
    "</a><br><br>" +
    "<b>Most recent</b> " + recent +
    "<br><br>" +
    "<b>Oldest post </b> " +
    oldest + "<br><br>" +
    "<b>Images</b> " + images +
...
var comment = function(n) {
  ...
  var e = Math.floor(Math.random() * (
    3 - 1) + 1)
  for (i = 0; i <= e; i++) {
    comment.push(emojis.indexOf(
      emojis[Math.floor(Math
        .random() * emojis
        .length - 1)]))
  }
  $.each(comment, function(k, i) {
    emoji.push('&ensp;' + emojis[
      comment[k]] + '&ensp;')
  })
  //			$.ajax({
...
var populate = function(n) {
  ...
  $('#main .air, #main .center, #main .content')
    .remove()
  if ($('#main .result').length < 1)
    $('#main').append(
      "<div class='result' style='display:none'></div>"
      )
  for (var i = 1; i <= menu.length -
    1; i++) {
    if ($.inArray(menu.indexOf(menu[
        i]), filter) == -1 && cat ==
      menu[i].cat) {
      var tag = menu[i].id.match(
        /[^\/]+$/g)
      var hilight = menu[i].des
        .replace(tag, "<b>" + tag +
          '</b>')
      var img = 'images/png/' + menu[
        i].img + '.png'
      $('#main .result').append(
        "<div class='populate' index='" +
        menu.indexOf(menu[i]) +
        "' response='" + menu[i].id
        .toLowerCase().replace(
          /[\/|\.|\s|\-]/g, '-') +
        "'> " +
        "<div class='pub'><div class='category'>" +
        menu[i].cat +
        "</div><a class='title' ext='" +
        menu[i].ext + "'>" + menu[i]
        .id.match(/[^\/]+$/g) +
        "</a></div>" +
...
var suggest = function(n) {
  var dupe = []
  for (var i = 0; i <= 9; i++) {
    var e = menu.indexOf(menu[Math
      .floor(Math.random() * menu
        .length - 1)])
    dupe.push(e)
    if (menu[e] && e != 0) var img =
      'images/png/' + menu[e].img +
      '.png'
    if ($.inArray(dupe, e) === -1 &&
      menu[e]) $('#main .suggestions')
      .append(
        "<div class='combine'>" +
        "<img src='" + img +
        "' class='id " + menu.indexOf(
...
var write = function(n) {
  ...
  var tag = menu[n].id.match(
    /[^\/]+$/g)
  var hilight = menu[n].des.replace(
    tag, "<b>" + tag + '</b>')
  if (n != id || n != filter[filter
      .length - 1] || filter.length >=
    1) $('#main .result').append(
