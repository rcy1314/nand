$(document)
  ...
  .on('touch click', '#arm #home',
    function(e) {
      window.location.href = window
        .location.origin
    })
  .on('touch click', '#option .fa-git',
    function(e) {
  ...
  .on('touch click', '.fa-times-circle',
    function(e) {
      $('#guide').remove()
    })
  .on('touch click', '.fa-globe',
    function(e) {
      window.location.href =
        'maintenance/rip.txt'
    })
  ...
  .on('touch click',
    '#main .center #bottom .previous',
    function(e) {
      exit('?q=&' + menu[$(this).attr(
          'index')].id.toLowerCase()
        .replace(/\s|\.|\//g, '-'))
    })
  .on('touch click',
    '#main .center #bottom .next',
    function(e) {
      exit('?q=&' + menu[$(this).attr(
          'index')].id.toLowerCase()
        .replace(/\s|\.|\//g, '-'))
    ...
    function(e) {
      ...
      if (location.href.match(
        ...
        if (location.href.match('\\+1'))
          var res = uri[0].replace(
            /\+1/g, '')
        else var res = uri[0]
        res = res.replace(/\-|\+/g, ' ')
        response(false, false, res,
          true, null)
        if (contrast == true && !
          location.href.match('\\+1'))
          uri = uri[0] + '+1'
        state('?q=' + uri[0].replace(
          /\-/g, '+'))
      }
    })
  .on(
    'keyup touch click focusin focusout blur',
    '#arm #search input[type=text]',
    function(e) {
      $this = $(this)
      if (e.type == 'focusin') {
  ...
  .on('submit',
    '#main .center .channel .item .classic .addComment',
    function(e) {
      if ($(this)
        .children('.comment')
        .val() != '') item = $(this)
  ...
  .on('touch click',
    '#main .center .quick .feed .asset .id',
    function(e) {
      var $this = $(this)
      $(this)
        .parent()
  ...
  .on('touch click',
    '#main .suggestions .combine div',
    function(e) {
      var uri = '?q=' + '&' + $(this)
        .attr('response')
      if (contrast == true && !location
        .href.match('\\+1')) uri = uri +
        '+1'
      else if (contrast == true) uri =
        uri + '+1'
      exit(uri)
  ...
  .on(
    'touch click mouseenter mouseleave',
  ...
  .on('touch click',
    ...
    function(e) {
      $(this)
        .toggleClass(
          'fa-circle-notch fa-circle')
      if (!location.href.match(
        '\\?q=') && !location.href
        .match('\\?\\+1') &&
        contrast == false) {
        var init = document.location
          .href + '?+1'
        state(init)
        contrast = true
        ...
        .match('\\+\\1') &&
        contrast == false) {
        var opposite = document.location
          .href + '+1'
        state(opposite)
        contrast = true
      } else if (location.href.match(
  ...
  .on('touch click', '#option .fa-code',
    function(e) {
      var re = menu.indexOf(menu[Math
        .floor(Math.random() * menu
          .length)])
      var uri = '?q=' + menu[re].cat
        .toLowerCase() + '&' + menu[re]
        .id.toLowerCase()
        .replace(/\s|\.|\//g, '-')
      if (contrast == true && !location
  ...
  .on('touch click',
    ...
    function(e) {
      ...
      var array = []
      for (i = 1; i <= menu.length -
        1; i++) {
        if (menu[i].cat == id) array
          .push(menu.indexOf(menu[i]))
      }
      var n = array[Math.floor(Math
        .random() * array.length)]
      var uri = '?q=&' + menu[n].id
        .toLowerCase()
        .replace(/(\s|\.|\/)/g, '-')
  ...
  .on('touch click',
    ...
    function(e) {
      $(this)
        .toggleClass(
          'fa-heart-o fab fa-gratipay')
      e.stopPropagation()
      visual()
    })
  .on('touch click',
    '#main .center .channel .item .header .fa-ellipsis-h, #container .sticky .header .fa-ellipsis-h',
    function(e) {
  ...
  .on('touch click',
    '#main .center .channel .item .image .img, #wrapper #container #guide .sticky .item .image .img',
    function(e) {
      if (tap == 0) {
        // set first click
        ...
        if (((new Date().getTime()) -
          ...
          visual()
          tap = 0;
        } else if (img == $(this).attr(
            'id')) {
          if ($(this)
            .hasClass('expand min') ||
            $(this)
            .hasClass('expand full'))
            expand($(this)
              .attr('id'))
          else if ($(this)
  ...
  .on('touch click',
    '#main .center .channel .item .pub .more',
    function(e) {
      $(this)
        .parent()
        .html($(this)
